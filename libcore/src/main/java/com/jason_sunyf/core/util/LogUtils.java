...
public class LogUtils {
     ...
     * 日志输出时的TAG
     */
    private static String mTag = "SINO";
    /**
     * 日志输出级别NONE
     */
...
//			FileUtils.writeFile(log + "\r\n", path, append);
    ...
    /**
     * Created by LWH at 2017/8/23 21:48
     * 拼接输出LOG
     */
    public static void printLog(String TAG, String value) {
        ...
        if (value.length() > 1000) {
            for (int i = 0; i < value.length(); i += 1000) {
                if (i + 1000 < value.length()) {
                    Log.i(TAG + "-" + i, value.substring(i, i + 1000));
                } else {
                    Log.i(TAG + "-" + i, value.substring(i, value.length()));
                }
            }
        } else {
            Log.i(TAG, value);
